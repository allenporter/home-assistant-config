...
async def mock_template(
    ...
    with AUTOMATION_YAML.open("r") as fd:
        ...
        content = content.replace("notify.discord", "notify.notify")
...
async def test_notify_agenda(
    ...
    events: list[Event[Mapping[str, Any]]],
) -> None:
    ...
    assert len(events) == 1
    event = events[0].as_dict()["data"]
    assert "Agenda" in event.get("title")

    # We're using the default agent for testing
    assert "couldn't understand that" in event.get("message")
